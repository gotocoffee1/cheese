...
public:
		...
		if (short_name != " " && valid_pre_conditions(stx, sty, destx, desty))
		{
			...
			else
			{
				...
				else
				{
					if (short_name == "B")
					{
						move_bishop(stx, sty, destx, desty);
					}
					else
					{
						if (short_name == "R")
						{
							move_rook(stx, sty, destx, desty);
						}
						else
						{
							if (short_name == "Q")
							{
								//move_queen(stx, sty, destx, desty);
							}
							else
							{
								if (short_name == "K")
	...
	void move_rook(int stx, int sty, int destx, int desty)
	{
		bool figure_in_the_way = false;
		int direction = sty;
