...
public:
	...
	chess_board()
	{
		...
		for (auto& row : board_field)
		{
			for (auto& column : row)
			{
				column.set_piece(new pawn(color::WHITE));
			}
		}
		*/
		...
		for (int i = 0; i < 8; i++)
		{
			for (int j = 0; j < 8; j++)
			{
				if (i < 2 || i > 6)
				{
					...
					board_field[7][0].set_piece(new rook(color::BLACK));
					board_field[7][1].set_piece(new knight(color::BLACK));
					board_field[7][2].set_piece(new bishop(color::BLACK));
					board_field[7][3].set_piece(new queen(color::BLACK));
					board_field[7][4].set_piece(new king(color::BLACK));
					board_field[7][5].set_piece(new bishop(color::BLACK));
					board_field[7][6].set_piece(new knight(color::BLACK));
					board_field[7][7].set_piece(new rook(color::BLACK));

				}
				else
				{
					board_field[i][j].set_piece(new empty(color::BLACK));
				}
			}
		}
	}
